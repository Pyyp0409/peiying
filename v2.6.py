...
def show_billing_portal():
    ...
    tab1, tab2 = st.tabs(["📋 Pending Invoices", "💰 Payment Processing"])
...
def show_vendor_portal():
    ...
    if vendor_info:
        ...
        <div class="card success-card">
            ...
            <p><strong>Service:</strong> {vendor_info['service']}</p>
            <p><strong>Status:</strong> {vendor_info['status']}</p>
            <p><strong>Since:</strong> {vendor_info['registration_date']}</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Vendor services
        st.markdown("#### Service Management")
        service_update = st.text_area("Service Description Update", 
                                     value=f"Currently providing {vendor_info['service']} services")
        
        if st.button("Update Service Description", use_container_width=True):
            st.success("Service description updated successfully!")
    
    else:
        st.warning("Your vendor application is still pending approval.")
