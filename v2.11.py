...
def add_notification(message, category="info", target_roles=None, target_user=None):
    ...
    notification = {
        ...
        "target_user": target_user  # NEW: Specific user targeting
    }
    st.session_state.notifications.append(notification)

# ADD this new function right after add_notification:
def get_user_notifications():
    """Get notifications relevant to current user with better filtering"""
    user_notifications = []
    ...
    for notification in st.session_state.notifications:
        ...
        # 3. No specific targeting (broadcast)
        if (target_user == current_email or 
            current_role in target_roles or 
            not target_roles):
            user_notifications.append(notification)
    
    return user_notifications[-10:][::-1]
...
def show_main_application():
    with st.sidebar:
        # User info card - KEEP THIS PART THE SAME
        ...
        # Notifications section - REPLACE FROM HERE DOWN
        user_notifications = get_user_notifications()  # Use the new function
...
def show_guest_booking():
    ...
    # NEW: Show room availability - ADD THIS SECTION
    ...
    # Payment confirmation
    if st.button("💳 Confirm Booking & Proceed to Payment", use_container_width=True):
        # NEW: Auto-assign room - ADD THESE LINES
        ...
        new_booking = {
            ...
            "guest_email": st.session_state.current_user['email'],
            "room_type": room_type,
            "room_number": assigned_room,  # NEW: Auto-assigned room
            "check_in": check_in.strftime("%Y-%m-%d"),
            "check_out": check_out.strftime("%Y-%m-%d"),
            "status": "Confirmed",
        ...
        # Update room status if room was assigned - ADD THIS BLOCK
        if assigned_room:
...
def show_room_availability_for_guest(check_in, check_out, room_type):
    ...
    available_rooms = [
        ...
        if room["type"] == room_type and room["status"] == "vacant"
    ]
    
    # Check if rooms are booked during the selected period
    conflicting_bookings = [
        booking for booking in st.session_state.bookings 
        if booking["room_type"] == room_type 
        and booking["status"] == "Confirmed"
        and not (check_out <= booking["check_in"] or check_in >= booking["check_out"])
    ]
    
    # Remove rooms that are booked during this period
    available_count = len(available_rooms) - len(conflicting_bookings)
    
    if available_count > 0:
        st.success(f"✅ {available_count} {room_type} room(s) available for your dates")
        return True
    else:
...
def assign_room_automatically(room_type, check_in, check_out):
    ...
    return available_rooms[0]["number"]
    ...
    for invoice in pending_invoices:
        ...
        with col2:
            if st.button("✅ Mark Paid", key=f"paid_{invoice['id']}"):
                ...
                st.rerun()
        with col3:
            # FIXED: Find guest email and send targeted reminder
            guest_email = None
            for booking in st.session_state.bookings:
                if booking["id"] == invoice["booking_id"]:
                    guest_email = booking["guest_email"]
            ...
            if st.button("📧 Send Reminder", key=f"remind_{invoice['id']}"):
                if guest_email:
                    # Send notification specifically to the guest
                    add_notification(
                        f"Payment reminder for invoice {invoice['id']}. Amount: ${invoice['amount']} due by {invoice['due_date']}",
                        "payment_reminder", 
                        target_roles=["Guest"],
                        target_user=guest_email  # Target specific guest
                    )
...
def show_vendor_payment_history():
    ...
    # Filter for ABC Laundry payments for September and October
    abc_laundry_payments = [s for s in st.session_state.vendor_statements 
                           if s["vendor_name"] == "ABC Laundry" 
                           and s["month"] in ["2024-09", "2024-10"]]
    
    if abc_laundry_payments:
        st.markdown("#### ABC Laundry - September & October 2024 Payments")
        ...
        fig = px.bar(x=months, y=amounts, 
                    title="ABC Laundry Payment Trend - Sep & Oct 2024",
                    labels={"x": "Month", "y": "Amount Paid ($)"},
                    color=months)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("No payment history found for ABC Laundry in September and October 2024.")

# ==================== MANAGER PORTAL ====================
def show_manager_portal():
    st.markdown('<div class="main-header">👨‍💼 Hotel Manager Dashboard</div>', unsafe_allow_html=True)
    
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([  # ADDED tab8
        "📈 Analytics", "👥 User Management", "🤝 Vendor Management", "📊 Reports", 
        "⚙️ Configuration", "📋 Approvals", "💰 Financial Overview", "👨‍💼 Staff Performance"  # NEW TAB
    ])
...
def show_manager_reports():
    ...
    if st.button("📥 Download Report", use_container_width=True):
        # Generate sample report data with real figures
        ...
        report_data = pd.DataFrame({
            'Metric': ['Total Revenue', 'Average Occupancy', 'RevPAR', 'Guest Satisfaction', 'Staff Performance', 'Vendor Expenses'],
...
def show_housekeeping_portal():
    ...
    tab1, tab2 = st.tabs(["📋 Assigned Tasks", "📊 Performance"])
